...
@media (prefers-reduced-motion: reduce) {
  body::after { animation: none; opacity: 0.5; }
    ...
    .iv-hero img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.9s cubic-bezier(0.2, 0.7, 0.3, 1); }
    .iv-hero:hover img { transform: scale(1.05); }
    .iv-hero-grad { position: absolute; inset: 0; background: linear-gradient(0deg, rgba(3,4,10,0.92) 0%, rgba(3,4,10,0.28) 46%, transparent 72%); }
    .iv-hero-cap { position: absolute; left: 24px; right: 24px; bottom: 20px; display: flex; flex-direction: column; gap: 6px; }
    ...
    /* Lightbox */
    .iv-lightbox { position: fixed; inset: 0; z-index: 1000; display: flex; align-items: center; justify-content: center; padding: 24px; background: rgba(2,3,8,0.93); -webkit-backdrop-filter: blur(6px); backdrop-filter: blur(6px); cursor: zoom-out; animation: ivLbIn 0.25s ease; }
    .iv-lightbox[hidden] { display: none; }
    .iv-lightbox img { max-width: min(94vw, 1100px); max-height: 90vh; border-radius: 12px; box-shadow: 0 30px 80px rgba(0,0,0,0.6); }
    ...
    <section class="section reveal hidden-view" id="artist" data-view="artist">
      ...
      <div class="tabs" id="artistTabs">
        ...
        <div class="tab-panel" data-panel="interview">
          ...
          <div class="iv-lightbox" id="ivLightbox" hidden aria-hidden="true"><img src="" alt=""></div>
        </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      try {
        ...
        // Interview : révélation au défilement (observer dédié) + lightbox des images
        (function () {
          const ivEls = document.querySelectorAll('.iv-reveal');
          ...
          if (lb) {
            ...
            document.querySelectorAll('.iv-shot img').forEach(img => {
