...
@media (prefers-reduced-motion: reduce) {
  body::after { animation: none; opacity: 0.5; }
    ...
    .tab-text { max-width: 760px; margin: 0 auto; text-align: center; }
    ...
    /* Cover hero */
    .iv-hero { position: relative; margin: 0 0 22px; border-radius: 18px; overflow: hidden; border: 1px solid var(--hairline); aspect-ratio: 16 / 9; }
    .iv-hero { cursor: zoom-in; }
    .iv-hero img { width: 100%; height: 100%; object-fit: cover; display: block; transform: translateY(0) scale(1.1); will-change: transform; }
    .iv-hero-grad { position: absolute; inset: 0; pointer-events: none; background: linear-gradient(0deg, rgba(3,4,10,0.92) 0%, rgba(3,4,10,0.28) 46%, transparent 72%); }
    .iv-hero-cap { position: absolute; left: 24px; right: 24px; bottom: 20px; display: flex; flex-direction: column; gap: 6px; pointer-events: none; }
    .iv-hero-kick { font-family: var(--font-body); font-size: 0.64rem; letter-spacing: 0.16em; text-transform: uppercase; color: var(--gold); }
    .iv-hero-title { font-family: var(--font-display); font-size: clamp(1.6rem, 5vw, 2.6rem); color: #fff; line-height: 1; text-shadow: 0 2px 24px rgba(0,0,0,0.5); }
    /* Meta chips */
    .iv-meta { display: flex; flex-wrap: wrap; gap: 8px; margin: 0 0 22px; }
    .iv-chip { font-family: var(--font-body); font-size: 0.66rem; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(233,240,255,0.78); padding: 6px 12px; border-radius: 999px; border: 1px solid var(--hairline); background: rgba(255,255,255,0.03); }
    /* Lede */
    .iv-article p.iv-lede { color: #eaf2ff; font-size: 1.18rem; line-height: 1.62; margin: 0 auto 8px; max-width: 720px; padding-bottom: 24px; border-bottom: 1px solid var(--hairline); }
    /* Q&A numérotées */
    .iv-qa { margin: 28px 0 0; }
    .iv-q { position: relative; font-family: var(--font-display); font-size: 1.12rem; color: #fff; margin: 0 0 10px; line-height: 1.32; text-transform: none; padding-left: 52px; }
    .iv-q::before { counter-increment: ivq; content: counter(ivq, decimal-leading-zero); position: absolute; left: 0; top: 1px; font-size: 0.92rem; letter-spacing: 0.06em; color: var(--primary-color); opacity: 0.9; }
    .iv-q::after { content: ''; position: absolute; left: 2px; top: 22px; width: 30px; height: 1px; background: linear-gradient(90deg, var(--primary-color), transparent); }
    .iv-a { padding-left: 52px; }
    .iv-qa:first-of-type .iv-a::first-letter { font-family: var(--font-display); float: left; font-size: 3.1rem; line-height: 0.82; padding: 4px 10px 0 0; color: #fff; }
    /* Pull-quotes */
    .iv-pull { position: relative; margin: 36px 0; padding: 6px 0 6px 28px; border-left: 3px solid var(--primary-color); font-family: var(--font-display); font-size: clamp(1.3rem, 3.2vw, 1.7rem); line-height: 1.4; color: #eaf2ff; text-transform: none; }
    .iv-pull::before { content: '“'; position: absolute; left: 12px; top: -8px; font-size: 2.4rem; color: rgba(102,191,255,0.4); }
    /* Feature & galerie + zoom au survol */
    .iv-shot { overflow: hidden; border-radius: 16px; border: 1px solid var(--hairline); cursor: zoom-in; background: rgba(20,22,38,0.5); }
    .iv-shot img { width: 100%; display: block; transition: transform 0.6s cubic-bezier(0.2, 0.7, 0.3, 1); }
    .iv-shot:hover img { transform: scale(1.07); }
    .iv-feature { margin: 34px 0; }
    .iv-feature figcaption { font-size: 0.82rem; color: var(--muted); text-align: center; margin-top: 11px; }
    .iv-feature figcaption em { color: #cfe9ff; }
    .iv-gallery { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin: 34px 0; }
    .iv-gallery .iv-shot { border-radius: 12px; }
    .iv-gallery .iv-shot img { aspect-ratio: 1; object-fit: cover; }
    .iv-gallery figcaption.iv-caption { grid-column: 1 / -1; font-size: 0.82rem; color: var(--muted); text-align: center; margin: 2px 0 0; }
    /* Source */
    .iv-article p.iv-source { margin: 38px auto 0; max-width: 720px; padding-top: 20px; border-top: 1px solid var(--hairline); font-size: 0.84rem; color: var(--muted); line-height: 1.6; }
    .iv-source a { color: var(--primary-color); }
    /* Bouton CTA "lire l'original" */
    .iv-cta { display: flex; width: fit-content; align-items: center; gap: 12px; margin: 36px auto 0; padding: 13px 22px; border-radius: 999px; font-family: var(--font-body); font-size: 0.84rem; letter-spacing: 0.06em; text-transform: uppercase; color: #eaf2ff; border: 1px solid rgba(102,191,255,0.4); background: linear-gradient(120deg, rgba(102,191,255,0.12), rgba(232,192,105,0.08)); transition: transform 0.25s ease, border-color 0.25s ease, box-shadow 0.25s ease; }
    .iv-cta:hover { transform: translateY(-2px); border-color: rgba(102,191,255,0.75); box-shadow: 0 12px 34px rgba(102,191,255,0.22); text-decoration: none; }
    .iv-cta-ic { display: inline-flex; color: var(--primary-color); }
    .iv-cta-ar { transition: transform 0.25s ease; }
    .iv-cta:hover .iv-cta-ar { transform: translateX(4px); }
    /* Barre de progression de lecture */
    .iv-progress { position: fixed; top: 0; left: 0; width: 100%; height: 3px; transform: scaleX(0); transform-origin: left; background: linear-gradient(90deg, var(--primary-color), var(--gold)); z-index: 900; opacity: 0; transition: opacity 0.3s ease; pointer-events: none; box-shadow: 0 0 10px rgba(102,191,255,0.5); }
    .iv-progress.show { opacity: 1; }
    /* Lightbox avec navigation */
    .iv-lightbox { position: fixed; inset: 0; z-index: 1000; display: flex; align-items: center; justify-content: center; padding: clamp(16px, 4vw, 56px); background: rgba(2,3,8,0.94); -webkit-backdrop-filter: blur(8px); backdrop-filter: blur(8px); animation: ivLbIn 0.28s ease; }
    .iv-lightbox[hidden] { display: none; }
    .iv-lb-fig { margin: 0; display: flex; flex-direction: column; align-items: center; gap: 14px; max-width: 100%; max-height: 100%; }
    .iv-lb-fig img { max-width: min(92vw, 1080px); max-height: 82vh; border-radius: 12px; box-shadow: 0 30px 90px rgba(0,0,0,0.65); animation: ivImgIn 0.3s ease; }
    .iv-lb-fig figcaption { font-size: 0.84rem; color: rgba(233,240,255,0.75); text-align: center; max-width: 640px; }
    .iv-lb-x { position: absolute; top: 18px; right: 20px; width: 42px; height: 42px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; color: #fff; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.14); cursor: pointer; transition: background 0.2s ease, transform 0.2s ease; }
    .iv-lb-x:hover { background: rgba(255,255,255,0.18); transform: rotate(90deg); }
    .iv-lb-nav { position: absolute; top: 50%; transform: translateY(-50%); width: 50px; height: 50px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; color: #fff; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.14); cursor: pointer; transition: background 0.2s ease; }
    .iv-lb-nav:hover { background: rgba(102,191,255,0.28); }
    .iv-lb-prev { left: 18px; } .iv-lb-next { right: 18px; }
    .iv-lb-count { position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); font-family: var(--font-body); font-size: 0.74rem; letter-spacing: 0.14em; color: rgba(233,240,255,0.6); }
    @keyframes ivLbIn { from { opacity: 0; } to { opacity: 1; } }
    @keyframes ivImgIn { from { opacity: 0; transform: scale(0.97); } to { opacity: 1; transform: none; } }
    @media (max-width: 600px) { .iv-lb-nav { width: 42px; height: 42px; } .iv-lb-prev { left: 8px; } .iv-lb-next { right: 8px; } }
    @media (max-width: 600px) {
      .iv-hero { aspect-ratio: 4 / 3; }
      .iv-hero-cap { left: 16px; right: 16px; bottom: 14px; }
      .iv-gallery { grid-template-columns: 1fr 1fr; }
      .iv-q, .iv-a { padding-left: 40px; }
      .iv-article p.iv-lede { font-size: 1.06rem; }
      .iv-pull { font-size: 1.2rem; }
    }
    @media (prefers-reduced-motion: reduce) {
      .iv-reveal { opacity: 1; transform: none; transition: none; }
      .iv-hero img, .iv-shot img { transition: none; }
    }
    ...
    .col-title-wrap {
      position: absolute; z-index: 2; inset: 0; padding: 0 372px 0 128px;
      display: flex; flex-direction: column; align-items: flex-start; justify-content: center; text-align: left; gap: 6px;
    }
    .col-title { margin: 0; font-size: clamp(1.05rem, 2vw, 1.5rem); line-height: 1.14; color: #fff; text-shadow: 0 2px 18px rgba(0, 0, 0, 0.9); }
    .col-date { font-size: 0.66rem; letter-spacing: 0.16em; text-transform: uppercase; color: rgba(233, 240, 255, 0.72); }
    .col-stats { position: absolute; z-index: 2; right: 48px; top: 50%; transform: translateY(-50%); display: flex; align-items: center; gap: 12px; }
    .col-stat { text-align: center; min-width: 42px; }
    .col-stat + .col-stat { border-left: 1px solid var(--hairline); padding-left: 12px; }
    .col-stat .cv { display: block; font-family: var(--font-display); font-size: 1.08rem; line-height: 1; color: var(--primary-color); text-shadow: 0 0 14px rgba(102, 191, 255, 0.35); }
    ...
    @media screen and (max-width: 600px) {
      .col-row { height: 132px; }
      .col-logo { width: 54px; height: 54px; left: 14px; top: 16px; transform: none; border-radius: 12px; box-shadow: 0 6px 18px rgba(0, 0, 0, 0.5), 0 0 0 3px rgba(11, 16, 48, 0.5); }
      .col-title-wrap { inset: auto; left: 80px; right: 14px; top: 15px; bottom: auto; padding: 0; align-items: flex-start; justify-content: flex-start; text-align: left; gap: 3px; }
      .col-title { font-size: 1rem; }
      .col-date { font-size: 0.54rem; letter-spacing: 0.12em; }
      .col-stats { position: absolute; left: 14px; right: 14px; bottom: 14px; top: auto; transform: none; gap: 0; display: grid; grid-template-columns: repeat(5, 1fr); }
      .col-stat { min-width: 0; }
      .col-stat + .col-stat { padding-left: 0; }
      .col-stat .cv { font-size: 0.9rem; }
      .col-stat .cl { font-size: 0.44rem; }
        ...
        <div class="tab-panel" data-panel="interview">
          <article class="iv-article">
            ...
            <div class="iv-meta iv-reveal">
              ...
              <span class="iv-chip">Medium</span>
            </div>
            <p class="iv-lede iv-reveal">“Creator Community” highlights some of the most interesting creators with collectibles available on the Crypto.com NFT platform.</p>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Where are you from, and what was your upbringing like?</h4>
              <p class="iv-a">I was born in a small industrial town in northern Spain, then I went to live in Madrid for 10 years. Now, I live in Barcelona. I grew up in a very normal, working-class family, but there were always books and music — and I was brought up in a very patient and sensitive way. I guess that influenced me to dedicate myself to something related to art.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">When did you first know you wanted to be a professional artist?</h4>
              <p class="iv-a">I always wanted to dedicate myself to something related to art and creativity, since what I liked to do most was to draw and tell stories. From a very young age, I was clear about my inability to dedicate myself to anything else. I always dreamed of being able to live without having to get up early — without having to do things out of obligation. Now I can say that I have achieved it. That makes me very happy.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Are you self-taught, or did you receive any training?</h4>
              <p class="iv-a">I studied graphic design in college and then I did several online and classroom courses. But I must say that almost everything I know, I know thanks to spending hours and hours of sleepless nights — for several years — reading in forums, watching tutorials and asking people who knew more than me. I strongly believe in self-learning. On the internet, we have access to all human knowledge for free. It’s just a matter of getting started.</p>
            </div>

            <blockquote class="iv-pull iv-reveal">On the internet, we have access to all human knowledge for free. It’s just a matter of getting started.</blockquote>

            <figure class="iv-feature iv-reveal">
              <div class="iv-shot"><img src="assets/interview/clouds.jpg" alt="Surreal collage of a silhouette before clouds and stars, by Fran Rodríguez" loading="lazy" decoding="async"></div>
              <figcaption>Fran works as <em>lacabezaenlasnubes</em> — “head in the clouds.”</figcaption>
            </figure>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Tell us about your professional experience.</h4>
              <p class="iv-a">I started working in advertising and marketing agencies as an art director in Madrid. After 10 years, I had a small existential crisis and decided I didn’t want to continue working for brands. It was a revelation. I quit my job and started working as a freelancer. The beginning was hard, but in a few years I managed to make a name for myself and projects started to come out. I have been living exclusively from my artistic work for 10 years. I accept some commissions, from time to time, because I love to design album covers — but I live from my personal work.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">How were you monetising your art before NFTs?</h4>
              <p class="iv-a">I was selling prints of my work. I also sold the rights to some of my work to be used in magazines or as book or album covers. I live on passive income that my work generates around the world.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">How did you get into NFTs?</h4>
              <p class="iv-a">I saw several artist friends starting to talk about NFTs on Instagram in February and March this year. I asked them and they told me. I spent several days studying and decided to give it a try. It’s the best decision I’ve ever made in my life.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What do you like most about working in the NFT space?</h4>
              <p class="iv-a">I like the freedom it gives artists — the opportunity to work and live off their artistic work without having to depend on other artists or brands or managers; I believe in decentralisation. I didn’t know anything about crypto seven months ago, but now I can say that these months have been some of the most exciting and interesting of my life. I have learned a lot and met some brilliant people.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Do you collect NFTs yourself?</h4>
              <p class="iv-a">Yes, I am starting a modest collection… I love being able to support artists I connect with.</p>
            </div>

            <blockquote class="iv-pull iv-reveal">I like the freedom it gives artists — the opportunity to work and live off your art, without depending on anyone.</blockquote>

            <figure class="iv-gallery iv-reveal">
              <div class="iv-shot"><img src="assets/interview/art-1.jpg" alt="Two figures on an orange dune beneath a floating shape, by Fran Rodríguez" loading="lazy" decoding="async"></div>
              <div class="iv-shot"><img src="assets/interview/art-2.jpg" alt="A levitating astronaut over a dune at sunset, by Fran Rodríguez" loading="lazy" decoding="async"></div>
              <div class="iv-shot"><img src="assets/interview/art-3.jpg" alt="Arches opening onto desert dunes at dusk, by Fran Rodríguez" loading="lazy" decoding="async"></div>
              <figcaption class="iv-caption">Stills from “Postcards From a Dream” and “Quantum Landscapes” by Fran Rodríguez.</figcaption>
            </figure>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Tell us about the NFTs in your first collection.</h4>
              <p class="iv-a">As the title indicates, they are postcards sent from dreams. They reflect various recurring dreams and visions that I have had, both asleep and awake — in a meditative and absorbed state. I would like those who see, and hopefully buy, the pieces to be transported to mental places where they are happy — and for them to look at reality with another perspective.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What do you consider “good” art?</h4>
              <p class="iv-a">For me, art has to be honest. Really honest. It must not respond to any practical or commercial purpose, nor be affected by fashions or trends. I like art that offers me an alternative way of seeing things and living — art that creates, somehow, a parallel universe.</p>
            </div>

            <blockquote class="iv-pull iv-reveal">For me, art has to be honest. Really honest. I like art that creates, somehow, a parallel universe.</blockquote>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What’s your favourite artistic movement?</h4>
              <p class="iv-a">Surrealism and, in general, any movement or artist who tries to reflect in their work their mind — their way of seeing and imagining the world. In short, all psychedelic art.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What’s more important: technique or message?</h4>
              <p class="iv-a">Message, no doubt. The technique, the way something is made, seems to me totally secondary. The emotion it conveys, the impact and the story it wants to tell is what is important.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What do you try to say with your art?</h4>
              <p class="iv-a">I try to transmit curiosity, amazement and fascination for nature — for the cosmos. That’s all that matters to me. I want people who see my work to feel a part of the magic I feel when I contemplate a sky or a mountain.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What are the common themes of your work?</h4>
              <p class="iv-a">Outer space, nature and the human figure facing the immensity of the cosmos. Mysterious shapes, geometries and people with their backs turned.</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">What inspires you, in general?</h4>
              <p class="iv-a">Nature. The universe. Everything that is not me inspires me — how the cosmos affects my mind, how it transforms it. I love the outdoors. Hiking. Music is a huge source of inspiration for me. I listen to music all the time, about eight or nine hours a day — in my studio, walking, cooking…</p>
            </div>

            <div class="iv-qa iv-reveal">
              <h4 class="iv-q">Who is your audience?</h4>
              <p class="iv-a">Any person who is self-absorbed, daydreaming all the time. Space lovers, psychonauts seeking altered states of consciousness and, in general, minds looking for something beyond the ordinary.</p>
            </div>

            <a class="iv-cta iv-reveal" href="https://medium.com/cryptocomnft/creator-community-fran-rodri%CC%81guez-f7f1734a3bd0" target="_blank" rel="noopener">
              <span class="iv-cta-ic"><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 5h12l4 4v10H4z"/><path d="M8 10h8M8 14h6"/></svg></span>
              <span class="iv-cta-tx">Read the original on Medium</span>
          ...
          <div class="iv-lightbox" id="ivLightbox" hidden aria-hidden="true">
            <button class="iv-lb-x" id="ivLbClose" type="button" aria-label="Close">
            ...
            <button class="iv-lb-nav iv-lb-prev" id="ivLbPrev" type="button" aria-label="Previous image">
              <svg viewBox="0 0 24 24" width="26" height="26" aria-hidden="true"><path d="M15 6l-6 6 6 6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
            <figure class="iv-lb-fig"><img src="" alt="" decoding="async"><figcaption id="ivLbCap"></figcaption></figure>
            <button class="iv-lb-nav iv-lb-next" id="ivLbNext" type="button" aria-label="Next image">
              <svg viewBox="0 0 24 24" width="26" height="26" aria-hidden="true"><path d="M9 6l6 6-6 6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
      ...
      try {
        ...
        a.innerHTML = \`
          ...
          <div class="col-stats">
            ...
            <div class="col-stat"><span class="cv">\${fmtMoney(st.floor)}</span><span class="cl">Floor</span></div>
            <div class="col-stat"><span class="cv">\${fmtMoney(st.vol)}</span><span class="cl">Volume</span></div>
            <div class="col-stat"><span class="cv">\${st.sales != null ? st.sales.toLocaleString('fr-FR') : '—'}</span><span class="cl">Sales</span></div>
          </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      try {
        ...
        (function () {
          ...
          const ivEls = article.querySelectorAll('.iv-reveal');
          if (!('IntersectionObserver' in window) || reduce) {
            ivEls.forEach(e => e.classList.add('iv-in'));
          } else {
            const ivObs = new IntersectionObserver((ents) => {
              ents.forEach(e => { if (e.isIntersecting) { e.target.classList.add('iv-in'); ivObs.unobserve(e.target); } });
            }, { threshold: 0.12, rootMargin: '0px 0px -8% 0px' });
            ivEls.forEach(e => ivObs.observe(e));
          }

          // 2) Parallaxe de la cover + barre de progression (un seul handler, throttlé en rAF)
          const heroImg = article.querySelector('.iv-hero img');
          const bar = document.getElementById('ivProgress');
          let ticking = false;
          function onScroll() {
            if (ticking) return; ticking = true;
            requestAnimationFrame(() => {
              ticking = false;
              if (article.offsetParent === null) { if (bar) bar.classList.remove('show'); return; }
              ...
              if (bar) {
                ...
                bar.classList.toggle('show', r.top < vh && r.bottom > 0);
              }
              if (heroImg && !reduce) {
                const hr = heroImg.parentElement.getBoundingClientRect();
                if (hr.bottom > 0 && hr.top < vh) {
                  const prog = (vh - hr.top) / (vh + hr.height);
                  heroImg.style.transform = 'translateY(' + ((prog - 0.5) * 28).toFixed(1) + 'px) scale(1.1)';
                }
              }
            });
          }
          ...
          // 3) Lightbox navigable (toutes les images de l'interview)
          const lb = document.getElementById('ivLightbox');
          if (lb) {
            const shots = Array.prototype.slice.call(article.querySelectorAll('.iv-hero img, .iv-shot img'));
            const lbImg = lb.querySelector('.iv-lb-fig img');
            const lbCap = document.getElementById('ivLbCap');
            const lbCount = document.getElementById('ivLbCount');
            let idx = 0;
            const show = (i) => {
              idx = (i + shots.length) % shots.length;
              const s = shots[idx];
              lbImg.src = s.currentSrc || s.src;
              lbImg.alt = s.alt || '';
              if (lbCap) lbCap.textContent = s.alt || '';
              if (lbCount) lbCount.textContent = (idx + 1) + ' / ' + shots.length;
            };
            const open = (i) => { show(i); lb.hidden = false; document.documentElement.style.overflow = 'hidden'; };
            const close = () => { lb.hidden = true; document.documentElement.style.overflow = ''; };
            shots.forEach((img, i) => { img.style.cursor = 'zoom-in'; img.addEventListener('click', () => open(i)); });
            const byId = (id) => document.getElementById(id);
            byId('ivLbClose').addEventListener('click', close);
            byId('ivLbPrev').addEventListener('click', (e) => { e.stopPropagation(); show(idx - 1); });
            byId('ivLbNext').addEventListener('click', (e) => { e.stopPropagation(); show(idx + 1); });
            lb.addEventListener('click', (e) => { if (e.target === lb || e.target.classList.contains('iv-lb-fig')) close(); });
