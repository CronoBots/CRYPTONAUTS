...
@media (prefers-reduced-motion: reduce) {
  body::after { animation: none; opacity: 0.5; }
    ...
    @media screen and (max-width: 600px) {
      .install-btn { font-size: 0.78rem; padding: 10px 14px; bottom: 16px; left: 16px; }
...
<body>
  ...
  <button id="installBtn" class="install-btn" hidden>📲 Installer l'app</button>

  <div id="installModal" class="modal" hidden role="dialog" aria-modal="true" aria-labelledby="installModalTitle">
    <div class="modal-box">
      <button class="modal-close" id="installModalClose" aria-label="Fermer">×</button>
      <h2 id="installModalTitle">📲 Installer l'application</h2>
      <p>Cryptonauts Leaderboards classe les détenteurs des collections NFT Cryptonauts (crypto.com). Ajoutez la page à votre écran d'accueil pour y accéder comme une vraie application — en plein écran, et même hors-ligne.</p>
      <button id="installNow" class="install-now" hidden>Installer maintenant</button>
      <h3>Sur iPhone / iPad (Safari)</h3>
      <ol>
        <li>Touchez le bouton <strong>Partager</strong> (carré avec une flèche ↑) en bas de l'écran.</li>
      ...
      <ol>
        <li>Touchez <strong>« Installer maintenant »</strong> ci-dessus, ou ouvrez le menu <strong>⋮</strong>.</li>
